...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            if 'min_loss' not in locals():
                min_loss = 100
                count_unimprovement = 0
            var1, var2 = self.train_epoch(dl_train)
            train_loss.extend(var1)
            train_acc.append(var2)
            var1, var2 = self.test_epoch(dl_val)
            val_loss.extend(var1)
            val_acc.append(var2)
            loss = sum(var1) / len(var1)
            if min_loss >= loss:
                min_loss = loss
                count_unimprovement = 0
            else:
                count_unimprovement += 1
                if count_unimprovement == early_stopping:
                    epoch = num_epochs
                    break
            # ========================
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        out_np = np.argmax(output.cpu().detach().numpy(),1)
        num_correct = np.sum(out_np == y_np)
        loss = loss.item()
        # ========================
