...
def run_experiment(run_name, out_dir='./results', seed=None,
    ...
    ds_train = CIFAR10(root='/results', download=True, train=True, transform=trans)
    ds_train, ds_val = train_test_split(ds_train, test_size = 0.15, random_state = 0)
    ds_test = CIFAR10(root='/results', download=True, train=False, transform=trans)
    ...
    in_size = x0.shape
    filters = []
    for num_of_filters_idx in filters_per_layer:
        for i in range(layers_per_block): filters.append(num_of_filters_idx)
    # model = torchvision.models.resnext50_32x4d(pretrained=True) #Q7
    # model = torchvision.models.resnet50(pretrained=True) #Q6
    # model.fc = nn.Linear(2048, 10) #Q6 + Q7
    model = model_cls(in_size, 10, filters, pool_every=pool_every, hidden_dims=hidden_dims) # Q1-Q5
    ...
    fit_res = trainer.fit(dl_train, dl_val, epochs, checkpoints, early_stopping, print_every=1,)
...
def load_experiment(filename):
    ...
    fit_res = train_results.FitResult(**output['results'])
